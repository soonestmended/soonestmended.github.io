...
function setWindowSize() {
    ...
    drawUniforms.u_resolution[0] = canvas.innerWidth*window.devicePixelRatio;
    drawUniforms.u_resolution[1] = canvas.innerHeight*window.devicePixelRatio; 
//    renderer.setSize(canvas.innerWidth, canvas.innerHeight);
}


var m4 = twgl.m4;
var gl = twgl.getWebGLContext(document.getElementById("codecanvas"));
...
texWidth = texHeight = 256; // dimensions of Perlin random vectors
...
// make Float32 array for Perlin noise vectors.
var texNoiseSrcArray = twgl.primitives.createAugmentedTypedArray(4, texWidth*texHeight, Float32Array);
...
for (var i = 0; i < texWidth * texHeight; i++) {
    ...
    texNoiseSrcArray.push([Math.cos(theta), Math.sin(theta), Math.cos(theta2), Math.sin(theta2)]);
}
...
var texNoise = twgl.createTexture(gl, {
    min: gl.NEAREST,
    mag: gl.NEAREST,
    width: texWidth,
    height: texHeight,
    src: texNoiseSrcArray,
});
...
// fill in drawUniforms.resolution above
//setWindowSize();

console.log(drawUniforms.u_resolution[0]);
...
var shadersToLoad = 2;
loadShader("/code/webgl/perlin.drawVS.glsl", function(xhttp) {drawVS = xhttp.responseText; checkLoaded();});
loadShader("/code/webgl/perlin.drawFS.glsl", function(xhttp) {drawFS = xhttp.responseText; checkLoaded();});
...
(function() {
    ...
    function handleMouseMove(event) {
        ...
        drawUniforms.center = [2.0*((event.clientX-rect.left) / gl.canvas.width)-1.0, 2.0*((rect.top-event.clientY)/ gl.canvas.height)+1.0];
...
(function() {
    ...
    function handleMouseDown(event) {
        ...
        drawUniforms.u_clicked = 1.0;
        drawUniforms.u_c2 = [1.0, 1.0, 1.0];
        drawUniforms.u_c1 = [0.1, 1.0, 0.1];
        //console.log(updateUniforms.center);
    }
})();
...
(function() {
    ...
    function handleMouseUp(event) {
        ...
        drawUniforms.u_clicked = -1.0;
        drawUniforms.u_c1 = [1.0, 1.0, 1.0];
        drawUniforms.u_c2 = [0.1, 1.0, 0.1];
        //console.log(updateUniforms.center);
    }
})();
...
(function() {
    ...
    function handleMouseWheel(event) {
        ...
        if (event.wheelDelta > 0) {
            drawUniforms.u_scale *= 0.9;
        }
        else {
            drawUniforms.u_scale *= 1.1;
        }
    }
})();
